...
export default function Breach() {
  ...
  const [prompt, setPrompt] = useState(null); // { char, spotIdx, id }
  ...
  function handleHit() {
    ...
    setScore(function (s) { return s + Math.round(10 * (1 + Math.min(combo, 20) * 0.08)); });
        ...
        <div style={{ position: "relative", zIndex: 1 }}>
              ...
              {TIERS.map(function (t, i) {
                ...
                return (
                  <button key={t.label} onClick={function () { setTierIdx(i); }} style={{
                    ...
                    background: active ? C.accentDark : C.card, border: "1px solid " + (active ? C.accent : C.border),
                    color: active ? C.accent : C.sub }}>
                    <span style={{ fontSize: 12, textTransform: "uppercase", letterSpacing: 0.3 }}>{t.label}</span>
                    <span style={{ fontSize: 10, color: C.mute }}>{tierBest ? "BEST " + tierBest : "—"}</span>
                  </button>
                );
              })}
          ...
          {phase === "playing" && (
            <div>
              ...
              <div onClick={function () { if (prompt) handleHit(); }} style={{
                ...
                {prompt && (
                  <div key={prompt.id} style={{
                    ...
                    <div style={{
                      ...
                      animation: "brRingShrink " + prompt.window + "ms linear forwards",
                    }} />
                    <div style={{
                ...
                <p style={{ position: "absolute", bottom: 8, left: 0, right: 0, textAlign: "center",
                  fontSize: 9, color: C.mute, margin: 0 }}>
